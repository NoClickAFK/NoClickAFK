(function(){
  ...
  const VERSION='4.5.2';
  const WORKFLOW_FIX_VERSION='4.5.8';
  ...
  const STATUS_ORDER={
    'Новый объект':1,'Связались с арендодателем':2,'Осмотр запланирован':3,'Осмотрен':4,
    'Собираем информацию':5,'Проверяем документы':6,'Ведём переговоры':7,'Выбран':8,
    'Кандидат':1,'Осмотрена':4,'Переговоры':7,'Оставить':8,'Исключить':9,'':99,
  };
  ...
  async function waitForBaseQueue(locationId,timeoutMs=5000){
    ...
    while(pendingLocation(locationId)&&Date.now()-started<timeoutMs)await new Promise(resolve=>setTimeout(resolve,25));
  }
  ...
  function enqueue(snapshot){
    ...
    const tracked=task.finally(()=>{if(queues.get(snapshot.locationId)===tracked)queues.delete(snapshot.locationId)});
  ...
  function capture(target,immediate=false){
    ...
    if(immediate){enqueue(snapshot).catch(console.error);return}
  ...
  async function flush(){
    ...
    if(hydration?.hydrateCard){for(const card of document.querySelectorAll('[data-location-card]'))await hydration.hydrateCard(card)}
    return true;
  }
  ...
  function compareOptionalDates(left,right,suffix=''){
    const leftValue=numericDate(left,suffix),rightValue=numericDate(right,suffix);
  ...
  function taskPresentation(data={}){
    ...
    if(task){
      const meta=[PRIORITY_LABELS[task.priority]||PRIORITY_LABELS.normal,TASK_STATUS_LABELS[task.status]||task.status,task.assignee?`Ответственный: ${task.assignee}`:'Ответственный не назначен',task.dueDate?`Срок: ${formatDueDate(task.dueDate)}`:'Без срока'];
  ...
  async function refreshNextTaskPanels(){
    ...
    for(const card of document.querySelectorAll('[data-location-card]')){
      ...
      const title=panel.querySelector('[data-next-task-title-v447]'),meta=panel.querySelector('[data-next-task-meta-v447]');
  ...
  function statusRank(value){return STATUS_ORDER[String(value||'')]??90}

  function applyStatusSort(){
    const root=document.getElementById('locations'),mode=document.getElementById('locationSortMode')?.value,metrics=window.BogatkaDecisionUI?.lastMetrics||[];
    ...
    const desired=items.map(metric=>metric.id),current=[...root.querySelectorAll(':scope > [data-location-card]')].map(card=>card.dataset.locationCard).filter(id=>desired.includes(id));
    if(current.length===desired.length&&current.every((id,index)=>id===desired[index]))return true;
    for(const metric of items){const card=root.querySelector(`[data-location-card="${CSS.escape(metric.id)}"]`);if(card)root.appendChild(card)}
    return true;
  }
  ...
  async function applyWorkflowIntegrity(){
    ...
    if(window.BogatkaLaunchGateV454?.ready)await window.BogatkaLaunchGateV454.renderAll();
    applyStatusSort();
  }
  ...
  function installUpdateSummaryWrapper(){
    ...
    if(current.__workflowIntegrityV458)return true;
    const wrapped=async function(...args){const result=await current(...args);await applyWorkflowIntegrity();return result};
    Object.assign(wrapped,current);
    wrapped.__workflowIntegrityV458=true;
  ...
  function scheduleWorkflowIntegrity(delay=40){
    ...
    workflowTimer=setTimeout(()=>{installUpdateSummaryWrapper();applyWorkflowIntegrity().catch(console.error)},delay);
  }

  function workflowAudit(){
    const dated=pickNextTask({tasks:[{id:'undated',title:'Без срока',priority:'normal',status:'todo',createdAt:'2026-01-01T00:00:00Z'},{id:'dated',title:'Со сроком',priority:'normal',status:'todo',dueDate:'2026-07-10',createdAt:'2026-06-01T00:00:00Z'}]});
    const compute=window.BogatkaDecisionEngine?.computeAll,failures=[];
  ...
  function install(){
    ...
    const listener=event=>{const target=event.target;if(!target?.dataset?.field)return;capture(target,event.type==='change'||event.type==='blur')};
    ...
    document.addEventListener('change',event=>{if(event.target?.id==='locationSortMode')setTimeout(()=>applyStatusSort(),0)});
  ...
  window.BogatkaDurableFieldsV452={version:VERSION,ready:true,FIELDS,capture,flush,get pendingWrites(){return queues.size+timers.size}};
  window.BogatkaWorkflowIntegrityV457={version:WORKFLOW_FIX_VERSION,ready:true,pickNextTask,taskPresentation,statusRank,applyStatusSort,stabilizeDecisionWrappers,refreshNextTaskPanels,audit:workflowAudit};
  if(typeof loadBogatkaPatch==='function'){
    ...
    loadBogatkaPatch('script',{src:'./opening-project-persistence-v455.js'});
