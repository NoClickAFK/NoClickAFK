...
function metricByLabel(data,pattern){return data.metrics.find(metric=>pattern.test(clean(metric.label)))?.value||''}
function scoreValue(data){return metricByLabel(data,/\/\s*70|балл/i)}
...
function buildPortfolioSummary(doc,oldSummary,locations){
  const datasets=locations.map(location=>contextData(location)),withDecision=datasets.filter(data=>data.decision).length,selected=datasets.filter(data=>/остав|выбр|приоритет/i.test(data.decision)).length,pending=locations.length-withDecision;
  const weighted=datasets.map(data=>parseNumber(weightValue(data))).filter(Number.isFinite),completion=datasets.map(data=>parseNumber(completionValue(data))).filter(Number.isFinite);
...
function locationInsight(location,data){
  const completion=parseNumber(completionValue(data));
...
function buildRiskOverview(doc,locations){
  const low=locations.map((location,index)=>({location,index,data:contextData(location)})).filter(item=>{const completion=parseNumber(completionValue(item.data));return completion===null||completion<35||!item.data.decision});
...
function makeLocationSummary(doc,location,index,open,dataOverride=null){
  ...
  const metrics=node(doc,'span','report-location-summary-metrics-v433');canonicalMetrics(data).forEach(item=>{const metric=node(doc,'span','report-location-summary-metric-v433');append(metric,node(doc,'strong','',item.value),node(doc,'small','',item.label));metrics.appendChild(metric)});
