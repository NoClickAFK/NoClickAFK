(function(){
  ...
  cloudApplyRemote=async function integrityApplyRemote(remoteLocations,remotePhotos,remoteState,syncState){
    ...
    for(const item of [...locations]){
      ...
      const dirty=(syncState.dirtyLocations||[]).includes(item.id)||Boolean(syncState.metaDirty);
      if(!row&&item.cloudId&&known.has(item.id)&&!dirty){
        locations=locations.filter(entry=>entry.id!==item.id);
        await State.rawDelete()(STORE,`location:${item.id}`);
    ...
    const safeState={...syncState,dirtyLocations:[...new Set([...contexts.keys(),...remoteLocations.map(row=>row.client_id||row.id)])],knownLocationIds:[]};
    await baseApplyRemote(remoteLocations,remotePhotos,remoteState,safeState);
    for(const context of contexts.values())if(!context.needsPush&&context.row)await saveBase(context.id,context.row);
  };

  cloudPushLocations=async function integrityPushLocations(remoteLocations,syncState){
    const finalRows=new Map(remoteLocations.map(row=>[row.client_id||row.id,row]));
    for(let index=0;index<locations.length;index++){
      ...
      if(row)finalRows.set(item.id,row);
    }
  ...
  window.BogatkaSyncIntegrity={
    ...
    principle:'three-way-field-merge-with-revision-checked-location-writes',
  };
})();
