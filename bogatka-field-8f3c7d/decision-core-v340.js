...
const item=locations[i],data=await getLocationData(item.id),items=photoMap.get(item.id)||[],categories=new Set(items.map(p=>p.category||'other')),w=weighted(data),c=completeness(data,categories),s=stopState(data),area=number(data?.tech?.totalArea),rent=number(data?.tech?.rentPerMonth);
