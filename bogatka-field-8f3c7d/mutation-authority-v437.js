(function(){
  ...
  const PUBLICATION_CONTROL_SELECTOR='#shareReportBtn,#cloudPublishBtn';
  const PUBLICATION_DENIED_MESSAGE='Публикация отчёта доступна владельцу и редактору проекта.';
  const MUTATING_CLICK_SELECTOR=[
    '#addLocationBtn','#saveLocationBtn','#deleteLocationBtn','#clearAllBtn','#importBtn','#importFile',
    '#shareReportBtn','#cloudPublishBtn',
  ...
  const MUTATING_CONTROL_SELECTOR='[data-location][data-field],[data-global],#addLocationBtn,#saveLocationBtn,#deleteLocationBtn,#clearAllBtn,#importBtn,#shareReportBtn,#cloudPublishBtn,[data-action="edit-location"],[data-action="clear-location"],[data-action="restore-location"],[data-action="archive-location"],[data-action="delete-location"],[data-photo-location],.photo-add,.photo-delete,[data-archive-restore],[data-archive-delete]';
