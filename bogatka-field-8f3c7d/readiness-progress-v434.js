(function(){
  ...
  const GROUP_META={
    inspection:{title:'Параметры осмотра',target:'inspection'},
    landlord:{title:'Арендодатель и условия',target:'landlord'},
    scores:{title:'Оценка локации',target:'scores'},
    technical:{title:'Технические и финансовые параметры',target:'technical'},
    photos:{title:'Фотографии по категориям',target:'photos'},
    checks:{title:'Проверки перед арендой',target:'checks'},
    conclusion:{title:'Предварительное решение по локации',target:'conclusion'},
  };
  ...
  function inspectionRequirements(data={}){
    const items=[
      simple('статус работы',data.status),
      simple('тип объекта',data.objectType),
      simple('дата осмотра',data.date),
      simple('время осмотра',data.time),
      simple('этаж / расположение',data.floorLocation),
      simple('состояние помещения',data.premiseCondition),
      simple('доступность помещения',data.premiseAvailability),
      simple('готовность собственника',data.landlordReadiness),
      simple('цель осмотра',data.inspectionPurpose),
      simple('итог осмотра',data.inspectionResult),
    ];
  ...
  function landlordRequirements(data={}){
    const items=[
      simple('собственник / организация',data.ownerName),
      simple('роль контактного лица',data.contactRole),
      simple('контактное лицо',data.contact),
      requirement('телефон, мессенджер или email',[data.contactPhone,data.contactMessenger,data.contactEmail].some(filled)),
      simple('источник объекта',data.objectSource),
    ];
  ...
  function technicalRequirements(data={}){
    return[
      simple('общая площадь',data?.tech?.totalArea),
      simple('аренда в месяц',data?.tech?.rentPerMonth),
      simple('электрическая мощность',data?.tech?.powerKw),
      simple('требуемая мощность',data?.tech?.requiredPowerKw),
      simple('режим работы',data?.tech?.openingHours),
      simple('коммунальные расходы',data?.tech?.utilities),
      simple('оценка ремонта',data?.tech?.repairEstimate),
    ];
  }

  function conclusionRequirements(data={}){
    return[requirement('предварительное решение',VALID_DECISIONS.has(String(data.decision||'').trim()))];
  }
  ...
  function requirementGroup(key,items,detail=''){
    ...
    return{
      key,title:meta.title,target:meta.target,weight:GROUP_WEIGHTS[key],requirements:normalized,
  ...
  function scoreGroup(data={}){
    ...
    const items=Object.keys(weights).map(key=>simple(key,data?.score?.[key]));
    const group=requirementGroup('scores',items);
  ...
  function photoGroup(metric={}){
    ...
    const total=Number(plan.requiredTotal||Object.values(PHOTO_PLAN).reduce((sum,value)=>sum+value,0));
    ...
    return{
      key:'photos',title:GROUP_META.photos.title,target:'photos',weight:GROUP_WEIGHTS.photos,
      done,total,ratio:total?done/total:1,percent:total?Math.min(100,Math.round(done/total*100)):100,
      missingCount,missingLabels,
      detail:missingCount?`Не хватает ${missingCount} фото до минимального фотоплана.`:'Минимальный фотоплан выполнен.',
    };
  }
  ...
  function checksGroup(metric={}){
    ...
    return{
      key:'checks',title:GROUP_META.checks.title,target:'checks',weight:GROUP_WEIGHTS.checks,
      done,total,ratio:total?done/total:1,percent:total?Math.round(done/total*100):100,
  ...
  function buildProgress(metric={}){
    ...
    const groups=[
      requirementGroup('inspection',inspectionRequirements(data)),
      requirementGroup('landlord',landlordRequirements(data)),
      scoreGroup(data),
      requirementGroup('technical',technicalRequirements(data)),
      photoGroup(metric),
      checksGroup(metric),
      requirementGroup('conclusion',conclusionRequirements(data)),
    ];
  ...
  function applyMetric(metric={}){
    ...
    if(api?.scoreAnalysis&&api?.recommendation){
      const analysis=api.scoreAnalysis(metric.data||{},window.BogatkaDecisionEngine?.WEIGHTS||{});
  ...
  function transformMetrics(metrics=[]){
    for(const metric of metrics)applyMetric(metric);
    return metrics;
  }
  ...
  function installApi(){
    ...
    if(typeof current==='function'&&current!==apiTransformTerminal){
      ...
      wrapped.__readinessProgressV434=true;
      wrapped.__base=current;
      api.transformMetrics=wrapped;
      apiTransformTerminal=wrapped;
    }
    return true;
  }
  ...
  function installEngineTerminal(){
    ...
    const wrapped=async function(...args){
      installPhotoPlan();
      return transformMetrics(await current.apply(engine,args));
    };
    Object.assign(wrapped,current);
    wrapped.__readinessProgressV434=true;
    wrapped.__base=current;
    engine.computeAll=wrapped;
    engineTerminal=wrapped;
    return true;
  }
  ...
  async function refresh(){
    installPhotoPlan();
    installApi();
    installEngineTerminal();
    enforceOptionalReason();
    refreshPhotoCopy();
    const summary=window.updateSummary||((typeof updateSummary==='function')?updateSummary:null);
    if(typeof summary==='function')await summary();
    else await window.BogatkaCardProgressV448?.renderAll?.();
    enforceOptionalReason();
    refreshPhotoCopy();
  }
  ...
  function install(){
    ...
    root.addEventListener('input',event=>{
      if(event.target?.matches?.('[data-field="listingUrl"],[data-field="objectSourceOther"],[data-field="inspectionPurpose"],[data-field="inspectionResult"],[data-field="decisionReason"]'))schedule(340);
    },true);
    root.addEventListener('change',event=>{
      if(event.target?.matches?.('[data-field="objectSource"],[data-field="decision"]'))schedule(140);
    },true);
    new MutationObserver(()=>{
      installPhotoPlan();
      installApi();
      installEngineTerminal();
      enforceOptionalReason(root);
      refreshPhotoCopy(root);
    }).observe(root,{childList:true,subtree:true});
    schedule(0);
    [100,300,700,1500,3000,6000].forEach(delay=>setTimeout(()=>schedule(0),delay));
  }

  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();
  window.addEventListener('load',()=>schedule(0),{once:true});
  window.BogatkaReadinessProgressV434={
    version:VERSION,ready:true,PHOTO_PLAN,GROUP_ORDER,GROUP_WEIGHTS,
    inspectionRequirements,landlordRequirements,technicalRequirements,conclusionRequirements,
    buildProgress,applyMetric,transformMetrics,installPhotoPlan,installApi,installEngineTerminal,refresh,
  };
})();
