(function(){
  ...
  const display=value=>filled(value)?escapeHtml(value):'—';
  let attempts=0;
  ...
  function trafficHtml(data={}){
    const rows=list(data.trafficMeasurements);
    const legacy=legacyTrafficRows(data);
    const measurements=rows.map((item,index)=>`<article class="public-traffic-card-v453"><h4>Замер ${index+1}${weekday(item.date)?` · ${escapeHtml(weekday(item.date))}`:''}</h4><div class="public-data-grid-v453">${Object.entries(TRAFFIC_LABELS).map(([key,label])=>{let value=item[key];if(key==='durationMinutes'&&filled(value))value=`${value} минут`;if(key==='parkingOccupiedPct'&&filled(value))value=`${value}%`;return`<div><b>${escapeHtml(label)}:</b> ${display(value)}</div>`;}).join('')}</div></article>`).join('');
    const old=legacy.length?`<details class="public-legacy-v453"><summary>Ранее сохранённые поля трафика — ${legacy.length}</summary><div class="public-data-grid-v453">${legacy.map(row=>`<div><b>${escapeHtml(row.label)}:</b> ${display(row.value)}</div>`).join('')}</div><p>Значения сохранены без преобразования.</p></details>`:'';
  ...
  function competitorsHtml(data={}){
    ...
    const cards=items.map(({title,item})=>`<article class="public-competitor-card-v453"><h4>${escapeHtml(title)}</h4><div class="public-data-grid-v453">${Object.entries(COMPETITOR_LABELS).map(([key,label])=>`<div><b>${escapeHtml(label)}:</b> ${display(item[key])}</div>`).join('')}</div></article>`).join('');
  ...
  function appendStructured(article,data={}){
    ...
    removeOldSection(article,'Полевой замер трафика');
    removeOldSection(article,'Конкуренты и окружение');

    const trafficHolder=document.createElement('div');
    trafficHolder.innerHTML=trafficHtml(data);
    const scoreHeading=[...body.querySelectorAll('h3')].find(node=>node.textContent.trim()==='Оценка локации');
    const traffic=trafficHolder.firstElementChild;
    if(scoreHeading)body.insertBefore(traffic,scoreHeading);else body.prepend(traffic);

    const competitorsHolder=document.createElement('div');
    competitorsHolder.innerHTML=competitorsHtml(data);
    const competitors=competitorsHolder.firstElementChild;
    const notes=body.querySelector('.notes-grid');
    if(notes)body.insertBefore(competitors,notes);else body.append(competitors);
    return true;
  }
