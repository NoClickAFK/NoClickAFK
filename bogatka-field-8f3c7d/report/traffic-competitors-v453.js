(function(){
  ...
  const VERSION='4.5.3';
  const LEGACY_TRAFFIC_FIELDS=[['weekdayMorning','Будни 08:00–10:00, человек'],['weekdayDay','Будни 12:00–14:00, человек'],['weekdayEvening','Будни 17:00–20:00, человек'],['weekendDay','Выходной 11:00–14:00, человек'],['weekendEvening','Выходной 16:00–19:00, человек'],['parkingOccupied','Занято парковочных мест, %'],['dogWalkers','Люди с собаками за 30 минут'],['competitorVisitors','Покупатели у конкурента за 30 минут']];
  ...
  function weekday(value){if(!value)return'';const date=new Date(`${value}T12:00:00`);return Number.isNaN(date.getTime())?'':date.toLocaleDateString('ru-RU',{weekday:'long'});}
  function legacyTrafficRows(data={}){const source=data.traffic||{};return LEGACY_TRAFFIC_FIELDS.filter(([key])=>filled(source[key])).map(([key,label])=>({label,value:source[key]}));}
  ...
  function competitorsHtml(data={}){
    const items=[];const first=data.competitor&&typeof data.competitor==='object'?data.competitor:{};
  ...
  function removeOldSection(article,title){const heading=[...article.querySelectorAll('h3')].find(node=>node.textContent.trim()===title);if(!heading)return;const next=heading.nextElementSibling;heading.remove();next?.remove();}
  function appendStructured(article,data={}){
    const body=article?.querySelector('.location-body');if(!body)return false;
  ...
  function install(){
    ...
    const wrapped=function(payload){const result=current(payload);const items=(Array.isArray(payload?.snapshot?.locations)?payload.snapshot.locations:[]).filter(location=>!location.form_data?.archivedAt);document.querySelectorAll('#reportRoot article.location').forEach((article,index)=>appendStructured(article,items[index]?.form_data||{}));return result;};
    wrapped.__trafficCompetitorsV453=true;wrapped.__base=current;window.renderReport=wrapped;try{renderReport=wrapped}catch(_){ }
    setTimeout(()=>{try{if(typeof loadReport==='function')loadReport()}catch(error){console.error(error);}},0);return true;
  }

  window.BogatkaPublicTrafficCompetitorsV453={version:VERSION,ready:true,LEGACY_TRAFFIC_FIELDS,legacyTrafficRows,trafficHtml,competitorsHtml,appendStructured,install};
  install();
})();
