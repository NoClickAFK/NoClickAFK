(function(){
  ...
  function launchProgress(metric){
    const items=metric.data.launchProject?.milestones||[];
  ...
  function head(key,label){
    ...
    return `<button type="button" data-compare-sort="${key}" class="${active?'active':''}">${esc(label)}<span>${arrow}</span></button>`;
  }
  ...
  function deal(metric){
    const tone=metric.dealGate.code==='blocked'?'block':metric.dealGate.code==='needs_formalization'?'risk':metric.dealGate.code==='confirmed'?'clear':'empty';
    return `<span class="compare-stop-v340 ${tone}" title="${esc(metric.dealGate.text)}">${esc(metric.dealGate.compactText)}</span>`;
  }
  function mini(value){return value?`<span class="compare-mini-score-v332">${value}</span>`:'<span class="compare-empty-v332">—</span>'}
  function row(metric){
    const data=metric.data;
    const calculated=economy(metric);
    const launch=launchProgress(metric);
    const tasks=openTasks(metric);
  ...
  async function render(){
    ...
    table.querySelectorAll('[data-compare-sort]').forEach(button=>button.addEventListener('click',()=>{
      const key=button.dataset.compareSort;
      if(state.key===key)state.direction=state.direction==='asc'?'desc':'asc';
      else{
        state.key=key;
        state.direction=['rank','dealGate','rent','rentPerSqm','rentBurden','payback','tasks'].includes(key)?'asc':'desc';
      }
      render().catch(console.error);
    }));
    table.querySelectorAll('[data-compare-location]').forEach(button=>button.addEventListener('click',()=>{
      const card=document.querySelector(`[data-location-card="${CSS.escape(button.dataset.compareLocation)}"]`);
      if(!card)return;
      card.scrollIntoView({behavior:'smooth',block:'start'});
      card.classList.add('compare-highlight-v332');
      setTimeout(()=>card.classList.remove('compare-highlight-v332'),1800);
    }));
  }

  async function updateSummaryV430Compare(){
