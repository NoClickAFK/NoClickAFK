...
function appUrl(accessToken=null){
  const build=window.BOGATKA_BUILD||FALLBACK_BUILD;
  const url=new URL(location.href);
  url.hash='';
  url.search='';
  url.searchParams.set('v',build.versionToken||String(build.version).replace(/\D/g,''));
...
function protectLegacyVersionWriters(){
  ...
  if(typeof legacyUpgrade==='function'&&!legacyUpgrade.__versionProtectedV426){
    const wrapped=function(...args){
      ...
      if(label)label.id='versionLabelProtectedV426';
      try{return legacyUpgrade.apply(this,args)}finally{
        if(label)label.id='versionLabel';
        applyBuild(window.BOGATKA_BUILD||FALLBACK_BUILD);
      }
    };
    wrapped.__versionProtectedV426=true;
    window.upgradeV22Controls=wrapped;
    try{upgradeV22Controls=wrapped}catch(_){}
  }
  const premium=function(){
    if(typeof window.enhancePasswordField==='function')window.enhancePasswordField();
    applyBuild(window.BOGATKA_BUILD||FALLBACK_BUILD);
  };
  window.enhancePremiumUi=premium;
  try{enhancePremiumUi=premium}catch(_){}
}
...
function applyBuild(build){
  ...
  window.enforceCurrentVersion=()=>applyBuild(window.BOGATKA_BUILD);
  try{enforceCurrentVersion=window.enforceCurrentVersion}catch(_){}
  window.getAccessLinkData=()=>window.BogatkaVersion.getAccessLinkData(localStorage.getItem('bogatka_access_token_v1'));
  ...
  if(label){
    ...
    if(!versionObserver){
      versionObserver=new MutationObserver(()=>{
        ...
        if(current&&label.textContent!==current)label.textContent=current;
      });
      versionObserver.observe(label,{childList:true,characterData:true,subtree:true});
    }
  }
...
function installVersionedBackup(){
  ...
  replacement.__versionedV426=true;
  window.exportBackup=replacement;
  try{exportBackup=replacement}catch(_){}
}
...
async function resolveRemoteBuild(){
  ...
  versionClient||=window.supabase.createClient(
    window.BOGATKA_SUPABASE.url,
    window.BOGATKA_SUPABASE.publishableKey,
...
export async function installVersionAuthority(){
  ...
  try{
    const build=await resolveRemoteBuild();
    applyBuild(build);
    await registerVersionedWorker(build);
    setTimeout(installVersionedBackup,100);
    return build;
  }catch(error){
    console.warn('Automatic version resolution is unavailable; cached version is used.',error);
    return initial;
  }
}