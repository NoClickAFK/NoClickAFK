...
if (!failures.length) {
  ...
  for(const marker of ['hydrateRow','object_type','form.objectType','form.status','wrapFetch','wrapApply','archive-state-v436.js','installArchiveGate','waitForArchiveReady','archiveStateGateV436','archiveBootstrapV436','ARCHIVE_READY_TIMEOUT_MS=30000','archiveBootstrapPersistent:true','archiveReadyTimeoutMs','archiveLoadFailedV436','archiveScriptFailures','script.onerror','script.remove()'])if(!syncCompat.includes(marker))failures.push(`sync-field-compat-v416.js is missing ${marker}`);
  ...
  for(const marker of ['delegatedToV436','lateLoadProtected','bogatka:cloud-archive-loaded','__cloudArchiveV400','ensureRuntimeWrappers'])if(!cloudArchive.includes(marker))failures.push(`cloud-archive-v400.js is missing ${marker}`);
