...
const required = [
  ...
  'location-panels-v419.js','location-panels-render-v419.js','location-panels-v419.css','startup-panel-authority-v437.js','startup-panel-authority-v437.css','location-global-v421.js','location-global-v421.css',
...
if (!failures.length) {
  ...
  if (!loader.includes("BOGATKA_VISIBLE_VERSION='4.3.7'")) failures.push('v23.js visible version fallback is not v4.3.7');
  if (loader.includes("src:'./compare-v340.js'")) failures.push('v23.js still loads the legacy comparison implementation');
  const index = read('index.html');
  if (!index.includes('./startup-panel-authority-v437.js') || !index.includes('./startup-panel-authority-v437.css')) failures.push('index.html does not load the v4.3.7 startup panel authority before application initialization');
  if (index.indexOf('./startup-panel-authority-v437.js') > index.indexOf('./report-v2.js')) failures.push('startup panel authority loads after report-v2.js');
  ...
  for (const file of ['critical-deal-schema-v430.js','critical-deal-v430.css','compare-v430.js','suite-core-v400.js','suite-ui-v400.js','archive-label-v400.js','backup-v400.js','cloud-archive-v400.js','archive-state-v436.js','address-fix-v400.js','viewer-extra-v400.js','report-v400.js','selftest-v400.js','sync-merge-v412.js','sync-state-v412.js','sync-runtime-v412.js','sync-ui-v412.js','sync-field-compat-v416.js','field-integrity-v416.js','object-type-normalize-v416.js','location-profile-v416.js','location-profile-v416.css','location-panels-v419.js','location-panels-render-v419.js','location-panels-v419.css','startup-panel-authority-v437.js','startup-panel-authority-v437.css','location-global-v421.js','location-global-v421.css','location-card-collapse-v422.js','location-card-collapse-v422.css','version-authority-v426.js','report-finalize-v431.js','report-finalize-v432.js','report-editorial-core-v433.js','report-editorial-single-v433.js','report-editorial-portfolio-v433.js','report-editorial-style-a-v433.js','report-editorial-style-b-v433.js','readiness-progress-v434.js','reset/index.html','reset/reset.js']) if (!serviceWorker.includes(file)) failures.push(`Service Worker does not cache ${file}`);
  ...
  if (!serviceWorker.includes("||'437'")) failures.push('Service Worker fallback build token is not v437');
  ...
  for (const marker of ["version:'4.3.7'","versionToken:'437'","sourceCommit:'9c9543fc05a31985aaf5237cec5124a53d984ed3'","functions.invoke('bogatka-version')",'window.BOGATKA_BUILD','window.BogatkaVersion','upgradeV22Controls','enhancePremiumUi','exportBackup','serviceWorker.register','bogatka_build_meta_v426']) if (!versionAuthority.includes(marker)) failures.push(`version-authority-v426.js is missing ${marker}`);
  if (versionAuthority.includes("version:'4.3.6'") || versionAuthority.includes("versionToken:'436'")) failures.push('version-authority-v426.js still contains stale v4.3.6 production markers');
  const accessVersion = read('access-version-v400.js');
  if (!accessVersion.includes("import('./version-authority-v426.js')") || !accessVersion.includes('installVersionAuthority') || !accessVersion.includes('4.3.7')) failures.push('access-version-v400.js does not activate the centralized 4.3.7 version authority');
  const startupAuthority=read('startup-panel-authority-v437.js');
  for(const marker of ['BogatkaPanelAuthorityV437','prepareLocalUi','startCloudBackground','__localFirstV437','panel-authority-ready-v437','Статус, формат, состояние помещения и следующий шаг.','Контакты и предварительные условия аренды.','MutationObserver','audit()'])if(!startupAuthority.includes(marker))failures.push(`startup-panel-authority-v437.js is missing ${marker}`);
  if(startupAuthority.includes('cloudFetchRemote=')||startupAuthority.includes('cloudApplyRemote=')||startupAuthority.includes('cloudPushLocations='))failures.push('startup panel authority modifies cloud synchronization transport ownership');
  const startupCss=read('startup-panel-authority-v437.css');
  for(const marker of ['html:not(.panel-authority-ready-v437)', '.panel-authority-v437', '.panel-authority-failed-v437'])if(!startupCss.includes(marker))failures.push(`startup-panel-authority-v437.css is missing ${marker}`);
