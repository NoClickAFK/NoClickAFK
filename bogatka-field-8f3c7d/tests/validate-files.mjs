...
if (!failures.length) {
  ...
  for(const marker of ['delegatedToV436','lateLoadProtected','bogatka:cloud-archive-loaded','__cloudArchiveV400','ensureRuntimeWrappers','fetchDelegated','archiveFetchSource','installFetchAuthority','__archiveInclusiveFetchV400'])if(!cloudArchive.includes(marker))failures.push(`cloud-archive-v400.js is missing ${marker}`);
