...
const TITLES={
  'lidskaya-34':'ул. Лидская, 34, ТЦ «Лидский»','belusha-41a':'ул. Белуша, 41А','repina-54':'ул. Репина, 54',
  'rumlevskiy-10':'Румлёвский проспект, 10','makarovoy-2':'ул. Валентины Макаровой, 2','molodaya-7a':'ул. Молодая, 7А, ЖК «Погораны»','magistralnaya-10':'ул. Магистральная, 10, ЖК «Мир»',
};
const ADDRESSES={
  'lidskaya-34':'Гродно, ул. Лидская, 34','belusha-41a':'Гродно, ул. Белуша, 41А','repina-54':'Гродно, ул. Репина, 54',
  'rumlevskiy-10':'Гродно, Румлёвский проспект, 10','makarovoy-2':'Гродно, ул. Валентины Макаровой, 2','molodaya-7a':'Гродно, ул. Молодая, 7А','magistralnaya-10':'Гродно, ул. Магистральная, 10',
};
const filledBase={
  status:'Осмотрена',objectType:'Торговый центр',date:'2026-07-07',time:'12:00',floorLocation:'1 этаж',premiseCondition:'Рабочее состояние',premiseAvailability:'Доступно',landlordReadiness:'Готов обсуждать',
  ownerName:'Арендодатель',contactRole:'Собственник',contact:'Контакт',contactPhone:'+375290000000',tech:{totalArea:'45',rentPerMonth:'1250',powerKw:'12',openingHours:'10:00-21:00',utilities:'180',repairEstimate:'3000'},
...
function remoteRows(){
  return DEFAULT_IDS.map((id,index)=>({
    id:`cloud-${id}`,project_id:'project-1',client_id:id,title:TITLES[id],address:ADDRESSES[id],note:'',status:locationData(id).status||null,object_type:locationData(id).objectType||null,
    form_data:locationData(id),sort_order:index,revision:1,created_at:'2026-07-07T08:00:00.000Z',updated_at:'2026-07-07T10:00:00.000Z',archived_at:null,
  }));
}
...
(function(){
  ...
  function resultFor(table,mode,op,payload){
    ...
    if(table==='project_members')return ok(mode==='single'?{role:'owner'}:[{user_id:'user-1',role:'owner',created_at:'2026-07-07T08:00:00.000Z'}]);
  ...
  function query(table){
    ...
    const api={
      ...
      upsert(payload){state.op='upsert';state.payload=payload;return api},update(payload){state.op='update';state.payload=payload;return api},delete(){state.op='delete';return api},insert(payload){state.op='insert';state.payload=payload;return api},
      single(){return Promise.resolve(resultFor(table,'single',state.op,state.payload))},maybeSingle(){return Promise.resolve(resultFor(table,'maybeSingle',state.op,state.payload))},then(resolve,reject){return Promise.resolve(resultFor(table,'many',state.op,state.payload)).then(resolve,reject)},
    };
    return api;
  }
  window.supabase={createClient(){return {
    auth:{getSession:async()=>({data:{session:{user:{id:'user-1',email:'owner@example.com',user_metadata:{display_name:'Owner'}}}},error:null}),onAuthStateChange(callback){setTimeout(()=>callback('INITIAL_SESSION',{user:{id:'user-1',email:'owner@example.com',user_metadata:{display_name:'Owner'}}}),0);return {data:{subscription:{unsubscribe(){}}}}},signOut:async()=>({error:null}),signInWithPassword:async()=>({error:null}),signUp:async()=>({data:{session:null},error:null})},
    rpc:async name=>name==='claim_bogatka_project'?ok('project-1'):ok(null),from:query,storage:{from(){return {download:async()=>ok(new Blob(['remote'],{type:'image/jpeg'})),remove:async()=>ok([])}}},
    channel(){const channel={on(){return channel},subscribe(){window.__bogatkaCloudMockState.subscribed=true;return channel}};return channel},removeChannel:async()=>ok(null),
  }}}
})();`}
...
async function installMockCloud(page){
  ...
  await page.addInitScript(({rows})=>{
    window.__bogatkaCloudMockState={
      ...
      remoteState:{project_id:'project-1',data:{},updated_at:'2026-07-07T10:00:00.000Z'},subscribed:false,
...
async function installTrace(page){
  await page.addInitScript(()=>{
    ...
    const mark=(type,detail={})=>trace.events.push({at:now(),type,...detail});
    ...
    function sample(reason){if(!appVisible())return;if(trace.firstVisibleAt===null){trace.firstVisibleAt=now();mark('app-visible')}trace.samples.push({at:now(),elapsed:Math.round((now()-trace.firstVisibleAt)*10)/10,reason,comparison:comparison(),badges:badges(),recentFunctions:trace.functions.slice(-12),recentEvents:trace.events.slice(-12)})}
    const wrapped=new Set();
    const wrapFunction=(name,fn,setter)=>{if(wrapped.has(name)||typeof fn!=='function'||fn.__startupCloudTraceV469)return;const wrappedFn=function(...args){const start=now();trace.functions.push({name,start,type:'start'});try{const result=fn.apply(this,args);if(result?.finally)return result.finally(()=>trace.functions.push({name,start,end:now(),type:'end'}));trace.functions.push({name,start,end:now(),type:'end'});return result}catch(error){trace.functions.push({name,start,end:now(),type:'throw',message:error?.message||String(error)});throw error}};Object.assign(wrappedFn,fn);wrappedFn.__startupCloudTraceV469=true;wrappedFn.__base=fn;setter(wrappedFn);wrapped.add(name)};
    const wrapGlobal=name=>wrapFunction(`window.${name}`,window[name],fn=>{window[name]=fn});
    const wrapMethod=(objectName,method)=>{const object=window[objectName];if(!object)return;wrapFunction(`${objectName}.${method}`,object[method],fn=>{object[method]=fn})};
    function installWrappers(){
      ['authorize','setAuthorizedShell','revealAuthorizedApp','applyVersion23Enhancements','ensureWorkflowEnhancements','cloudInit','cloudSyncAll','cloudApplyRemote','cloudFetchRemote','cloudSetStatus','cloudRenderModal','cloudSubscribeRealtime','cloudHandleRealtime','restoreAllForms','renderLocations','updateSummary'].forEach(wrapGlobal);
      [['BogatkaStartup','prepareCriticalUi'],['BogatkaStartup','revealApp'],['BogatkaDecisionUI','refresh'],['BogatkaCardProgressV448','renderAll'],['BogatkaCardProgressV448','transformMetrics'],['BogatkaCardEnhancer','enhanceAll'],['BogatkaLocationCardCollapseV422','enhanceAll'],['BogatkaWorkflowV414','enhanceAll'],['BogatkaUIRefineV462','completeRuntime'],['BogatkaCloudStability','queueAutomaticSync']].forEach(([objectName,method])=>wrapMethod(objectName,method));
    }
    const observer=new MutationObserver(records=>{for(const record of records){const target=record.target instanceof Element?record.target:null;if(target?.id==='app'&&record.attributeName==='class')mark('app-class',{className:target.className,visible:appVisible()});if(target&&(target.id==='locationComparisonPanel'||target.closest?.('#locationComparisonPanel')))mark('comparison-mutation',{attribute:record.attributeName||null,targetId:target.id||'',targetClass:target.className||'',panelClass:document.getElementById('locationComparisonPanel')?.className||'',panelOpen:Boolean(document.getElementById('locationComparisonPanel')?.open)});for(const node of record.addedNodes){if(!(node instanceof Element))continue;if(node.id==='locationComparisonPanel'||node.querySelector?.('#locationComparisonPanel'))mark('comparison-inserted');if(node.matches?.('[data-card-recommendation-v448]')||node.querySelector?.('[data-card-recommendation-v448]'))mark('recommendation-inserted')}}sample('mutation')});
...
function assertTrace(history,label){
  ...
  expect(first.comparison.bodyHidden,`${label}: comparison body hidden`).toBe(true);
  ...
  for(const entry of first.badges){expect(entry.nodes.length,`${label}: one badge for ${entry.locationId}`).toBe(1);firstBadges.set(entry.locationId,{nodeId:entry.nodes[0].nodeId,signature:badgeSignature(entry.nodes[0])})}
  ...
  for(const [index,sample] of history.samples.entries()){
    ...
    for(const entry of sample.badges){
      ...
      const firstBadge=firstBadges.get(entry.locationId);if(!firstBadge)continue;
  ...
  const postRevealRender=history.functions.filter(call=>call.name==='window.renderLocations'&&call.start>history.firstVisibleAt+1);
...
async function bootAndCapture(browserType,viewport,label){
  ...
  try{
    ...
    await page.evaluate(async()=>{
      ...
      window.__bogatkaCloudMockState.bumpPhoto();
      window.cloudHandleRealtime?.();
      window.cloudHandleRealtime?.();
    });
...
test('Chromium cloud startup remains stable for 20 seconds after reveal',async()=>{await repeatedMatrix(chromium,'chromium')});
test('WebKit cloud startup remains stable for 20 seconds after reveal',async()=>{await repeatedMatrix(webkit,'webkit')});
