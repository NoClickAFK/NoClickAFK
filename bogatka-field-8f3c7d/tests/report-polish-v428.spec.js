...
test('report keeps compact comparison and removes workflow-only sections',async({page})=>{
  ...
  const result=await page.evaluate(async()=>{
    ...
    return {
      ...
      taskExamples:doc.querySelectorAll('.task-examples-v414,.task-form-help-v414').length,
      ...
      collaborationTitle:first?.querySelector('.collaboration-v400>summary')?.textContent?.trim()||'',
...
test('report score, collaboration and decision blocks use separated grid layouts',async({page,context})=>{
  ...
  const styles=await reportPage.evaluate(()=>{
    ...
    return {
      ...
      collaboration:styleOf('.collaboration-v400>.details-body'),
