...
test('v451 quick checklist loads after stage 4 and is cached for offline use',async({page})=>{
  ...
  await page.waitForFunction(()=>Boolean(
    ...
    window.saveField?.__fieldIntegrityV416
  ),{timeout:30000});
  ...
  const integration=await page.evaluate(async()=>{
    ...
    return{
      ...
      obsoleteScript:scripts.some(src=>src.includes('quick-checklist-stability-v451.js')),
      obsoleteCache:worker.includes('./quick-checklist-stability-v451.js'),
      renderVersion:window.BogatkaLocationPanelsRenderV419?.version,
      saveQueueIntact:Boolean(window.saveField?.__fieldIntegrityV416),
    };
  });

  expect(integration.stage4).toBeGreaterThanOrEqual(0);
  expect(integration.stage5).toBeGreaterThan(integration.stage4);
  expect(integration.stage5Report).toBeGreaterThan(integration.stage5);
  ...
  expect(integration.obsoleteScript).toBe(false);
  expect(integration.obsoleteCache).toBe(false);
  expect(integration.renderVersion).toBe('4.5.1');
  expect(integration.saveQueueIntact).toBe(true);
});
