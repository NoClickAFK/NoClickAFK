...
const SECTION_COPY='Контакты и предварительные условия аренды.';
...
test('landlord panel separates contact facts from lease proposal',async({page})=>{
  ...
  await expect(panel.locator('.panel-copy-v419,.profile-section-head-v416 > span')).toHaveText(SECTION_COPY);
  ...
  expect(await page.evaluate(()=>window.BogatkaLandlordConditionsV449.audit())).toMatchObject({ok:true});
});
