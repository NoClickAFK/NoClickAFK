...
async function confirmedChecks(page,id){
...
test('status or decision does not create an opening project automatically',async({page})=>{
  let card=await openApp(page);
  const id=await card.getAttribute('data-location-card');
  await resetLocation(page,id,{decision:'',status:'Кандидат',launchProject:undefined});
  card=page.locator(`[data-location-card="${id}"]`);
  const decision=card.locator('[data-field="decision"][value="Оставить"]');
  await decision.check();
  await page.waitForFunction(async locationId=>(await getLocationData(locationId)).decision==='Оставить',id);
...
test('opening project stays empty until the decision is keep',async({page})=>{
  let card=await openApp(page);
  const id=await card.getAttribute('data-location-card');
  await resetLocation(page,id,{decision:'Под вопросом',launchProject:{enabled:true,stage:'Ремонт',milestones:[{id:'legacy',title:'Существующий этап',status:'doing',order:0}]}});
  card=page.locator(`[data-location-card="${id}"]`);
  ...
  const stored=await page.evaluate(locationId=>getLocationData(locationId),id);
  expect(stored.launchProject.milestones[0].title).toBe('Существующий этап');
});

test('keep decision still requires all pre-lease checks',async({page})=>{
  let card=await openApp(page);
  const id=await card.getAttribute('data-location-card');
  await resetLocation(page,id,{decision:'Оставить',criticalDealConditions:{},launchProject:undefined});
  card=page.locator(`[data-location-card="${id}"]`);
  await page.evaluate(()=>window.BogatkaLaunchGateV454.renderAll());
  const details=card.locator('[data-launch-details]');
...
test('eligible location activates opening project only by explicit action',async({page})=>{
  let card=await openApp(page);
  const id=await card.getAttribute('data-location-card');
  await confirmedChecks(page,id);
  card=page.locator(`[data-location-card="${id}"]`);
  await page.evaluate(()=>window.BogatkaLaunchGateV454.renderAll());
  const details=card.locator('[data-launch-details]');
  await expect(details).toHaveAttribute('data-launch-gate-v454','ready');
  await details.evaluate(node=>node.open=true);
  await details.locator('[data-launch-activate-v454]').click();
  await page.waitForFunction(async locationId=>(await getLocationData(locationId)).launchProject?.enabled===true,id);
  const stored=await page.evaluate(locationId=>getLocationData(locationId),id);
...
test('reports hide legacy opening project when gate is not passed',async({page})=>{
