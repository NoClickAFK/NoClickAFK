...
test('transport normalization matches JSON semantics without losing explicit resets',async({page})=>{
  ...
  const result=await page.evaluate(()=>{
    ...
    const input={
      removed:undefined,empty:'',nil:null,flag:false,count:0,reset:[],nested:{keep:'yes',drop:undefined},
      ids:[{id:'task-1',title:'A'},{id:'task-2',deleted:false}],arrayValues:[undefined,false,0,'',null],deletedTaskIds:['task-3'],
    };
  ...
  expect(result.normalized).toEqual({
    empty:'',nil:null,flag:false,count:0,reset:[],nested:{keep:'yes'},
    ids:[{id:'task-1',title:'A'},{id:'task-2',deleted:false}],arrayValues:[null,false,0,'',null],deletedTaskIds:['task-3'],
  });
...
test('database no-op at revision 7403 is accepted once and saves the remote base',async({page})=>{
  ...
  const result=await page.evaluate(async({fixture})=>{
    ...
    const initial={
      id:'lidskaya-34',item:{id:'lidskaya-34',title:'ул. Лидская, 34, ТЦ «Лидский»'},index:0,row:fixture,base:null,local:payloadForm,merged:payloadForm,
      payload:{project_id:fixture.project_id,client_id:fixture.client_id,title:fixture.title,address:fixture.address,note:null,status:fixture.status,object_type:fixture.object_type,form_data:payloadForm,sort_order:0,archived_at:null,updated_by:'fixture-user'},
    ...
    const returned=await api.persistLocation(initial,{dirtyLocations:['lidskaya-34']},{
      isPending:()=>false,conditionalUpdate:async()=>{calls.patch++;return null},upsert:async()=>{throw new Error('upsert not expected')},
      fetchRow:async()=>{calls.get++;return fixture},rebuild:async()=>{calls.rebuild++;throw new Error('rebuild not expected')},
      saveLocal:async()=>{calls.saveLocal++},saveBase:async(_context,current)=>{calls.saveBase++;savedRevision=current.revision},
    });
  ...
  const result=await page.evaluate(async()=>{
    ...
    async function runCase(sameField){
      ...
      const returned=await api.persistLocation(makeContext(first,localForm),{dirtyLocations:[item.id]},{
        isPending:()=>false,
        conditionalUpdate:async(contextRow,payload)=>{revisions.push(contextRow.revision);if(contextRow.revision===10)return null;return {...advanced,form_data:payload.form_data,revision:12,updated_at:'2026-07-11T00:00:12.000Z'}},
        upsert:async()=>{throw new Error('upsert not expected')},fetchRow:async()=>{fetches++;return advanced},
        rebuild:async(_context,current)=>makeContext(current,Merge.merge(baseForm,localForm,current.form_data,{preferLocal:true})),
        saveLocal:async()=>{},saveBase:async()=>{},
      });
  ...
  const result=await page.evaluate(async({fixture})=>{
    ...
    try{
      await api.persistLocation(makeContext(),{dirtyLocations:['lidskaya-34']},{
        isPending:()=>false,conditionalUpdate:async()=>{patches++;return null},upsert:async()=>null,fetchRow:async()=>fixture,
        rebuild:async()=>makeContext(),saveLocal:async()=>{},saveBase:async()=>{},
      });
...
test('single-flight coalesces local and realtime requests into one follow-up without parallel calls',async({page})=>{
  ...
  const result=await page.evaluate(async()=>{
    const releases=[];
    const flight=window.BogatkaSyncCompatibility._test.createSingleFlight(async()=>{await new Promise(resolve=>releases.push(resolve))});
    const first=flight.invoke();
    await new Promise(resolve=>setTimeout(resolve,0));
    const second=flight.invoke();const third=flight.invoke();
    releases.shift()();
    while(!releases.length)await new Promise(resolve=>setTimeout(resolve,0));
    releases.shift()();
...
test('cloud modal shows one primary error and clears it after successful retry',async({page})=>{
  ...
  expect(await page.locator('#cloudModal').getByText(message,{exact:true}).count()).toBe(1);
