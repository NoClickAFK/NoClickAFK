...
test('clean initial synchronization exposes the exact no-edit push predicate',async({page})=>{
  test.setTimeout(45000);
  await authorize(page);
  await page.goto(APP,{waitUntil:'load'});
  await page.waitForFunction(()=>Boolean(
    window.BogatkaInitialBackgroundEditProtectionV437?.ready&&
    window.BogatkaSyncCompatibility?.ready&&
    window.BogatkaSyncState?.ready&&
    window.BogatkaArchiveStateV436?.ready&&
    window.BogatkaSyncFieldCompatV416?.archiveFetchReady
  ),{timeout:30000});
  const result=await page.evaluate(async()=>{
    const id='clean-push-diagnostic-v437';
    const projectId='project-clean-push-diagnostic-v437';
    const State=window.BogatkaSyncState;
    const Protection=window.BogatkaInitialBackgroundEditProtectionV437;
    const Compat=window.BogatkaSyncCompatibility;
    Protection._test.resetForTest();
    clearTimeout(cloudSyncTimer);
    clearTimeout(cloudRealtimeTimer);
    cloudProjectId=projectId;
    cloudSession={user:{id:'diagnostic-user-v437'}};
    cloudRole='owner';
    window.cloudRole=cloudRole;
    cloudSyncing=false;
    cloudApplyingRemote=false;
    const item={id,title:'Diagnostic location',address:'Diagnostic address',note:'Diagnostic note',custom:true};
    locations=[item];
    const localRow={contact:'LOCAL-STALE',questions:'LOCAL-STALE-UNTOUCHED',updatedAt:'2026-07-13T10:00:00.000Z'};
    const remoteRow={
      id:`remote-${id}`,project_id:projectId,client_id:id,title:item.title,address:item.address,note:item.note,
      status:null,object_type:null,form_data:{contact:'REMOTE-NEWER',questions:'REMOTE-NEWER-UNTOUCHED'},
      sort_order:0,archived_at:null,revision:7,updated_at:'2026-07-13T10:05:00.000Z',created_at:'2026-07-13T09:00:00.000Z',
    };
    const fixtureState=()=>({
      dirtyLocations:[],dirtyPhotos:[],deletedPhotos:{},deletedLocations:{},knownLocationIds:[id],knownPhotoIds:[],
      stateDirty:false,metaDirty:false,projectId,userId:cloudSession.user.id,
    });
    ...
    const stateAtStart=structuredClone(cloudReadState());
    await cloudApplyRemote([structuredClone(remoteRow)],[],null,stateAtStart);
    const stateAfterApply=structuredClone(cloudReadState());
    const context=await Compat._test.buildContext(item,0,structuredClone(remoteRow),stateAtStart);
    const baseAfter=await State.readBase(id);
    const localAfter=await getLocationData(id);
    const desired=Compat._test.comparable(context.payload);
    const actual=Compat._test.comparable(remoteRow);
    const differencePaths=Compat._test.differencePaths(desired,actual);
    const journalIds=Protection._test.journalIds();
    const localUpdatedAt=localAfter?.updatedAt||null;
    const remoteUpdatedAt=remoteRow.updated_at||null;
    const localNewer=Boolean(localUpdatedAt&&remoteUpdatedAt&&new Date(localUpdatedAt).getTime()>new Date(remoteUpdatedAt).getTime());
    const dirtyBeforePush=[...(stateAfterApply.dirtyLocations||[])];
    return{
      head:null,
      fixture:'synthetic-clean-no-edit',
      dirtyLocationsAtSyncStart:[...(stateAtStart.dirtyLocations||[])],
      dirtyLocationsAfterRemoteApply:[...(stateAfterApply.dirtyLocations||[])],
      dirtyLocationsBeforePush:dirtyBeforePush,
      metaDirty:Boolean(stateAtStart.metaDirty),
      stateDirty:Boolean(stateAtStart.stateDirty),
      startupDirtyMember:(Protection.snapshot.preExistingDirty||[]).includes(id),
      journalMember:journalIds.has(id),
      skipFirstPushMember:Protection._test.skipFirstPushIds.includes(id),
      followUpScheduledMember:Boolean(Protection.diagnostics.followUpSyncsScheduled),
      persistedBaseBeforeApply:baseBefore?{present:true,revision:baseBefore.revision}:null,
      persistedBaseAfterApply:baseAfter?{present:true,revision:baseAfter.revision}:null,
      localUpdatedAt,
      remoteUpdatedAt,
      localNewer,
      contextDirty:Boolean(context.dirty),
      contextNeedsPush:Boolean(context.needsPush),
      payloadDifferencePaths:differencePaths,
      pushReasons:{
        missingRemote:false,
        dirtyLocations:dirtyBeforePush.includes(id),
        metaDirty:Boolean(stateAtStart.metaDirty),
        stateDirty:Boolean(stateAtStart.stateDirty),
        localNewer,
        journalOrFollowUp:journalIds.has(id)||Protection._test.skipFirstPushIds.includes(id)||Boolean(Protection.diagnostics.followUpSyncsScheduled),
        payloadDifference:Boolean(context.needsPush),
      },
      internalIdbWriteMarkedDirty:dirtyBeforePush.includes(id)&&!(stateAtStart.dirtyLocations||[]).includes(id),
      lifecycle:Protection.lifecycle,
      generation:Protection.generation,
    };
  });
  result.head=EXACT_HEAD;
  await writeJson('05-clean-no-edit-push-diagnostic.json',result);
  expect(result.contextNeedsPush,JSON.stringify(result,null,2)).toBe(false);
});
