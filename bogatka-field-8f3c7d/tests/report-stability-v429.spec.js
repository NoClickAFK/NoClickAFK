...
async function openApp(page){
  await page.route('**/functions/v1/bogatka-version',route=>route.fulfill({
    ...
    body:JSON.stringify({version:'4.2.8',versionToken:'448',sourceCommit:'report448abcdef',ahead:1}),
  }));
...
test('report header keeps a compact textual recommendation to the right of the address',async({page,context})=>{
  await openApp(page);
  ...
  const layout=await reportPage.evaluate(()=>{
    ...
    const recommendation=document.querySelector('.report-head-recommendation-v448');
    const computed=head?getComputedStyle(head):null;
    const recommendationStyle=recommendation?getComputedStyle(recommendation):null;
    ...
    return {
      ...
      head:rect(head),
      title:rect(title),
      recommendation:rect(recommendation),
      recommendationBorder:recommendationStyle?.borderTopWidth||'',
      metricCount:document.querySelectorAll('.report-head-metric-v428').length,
      progressCount:document.querySelectorAll('.report-card-progress-v448').length,
    };
  });

  expect(layout.recommendation).toBeTruthy();
  expect(layout.metricCount).toBe(0);
  expect(layout.progressCount).toBeGreaterThan(0);
  expect(layout.columns.split(' ').length).toBe(2);
  expect(Math.abs(layout.recommendation.y-layout.title.y)).toBeLessThanOrEqual(2);
  expect(layout.recommendation.x).toBeGreaterThan(layout.title.x);
  expect(layout.recommendation.width).toBeGreaterThanOrEqual(250);
  expect(layout.recommendation.width).toBeLessThanOrEqual(318);
  expect(Math.abs((layout.head.right-layout.recommendation.right)-layout.paddingRight)).toBeLessThanOrEqual(2);
  expect(layout.recommendation.height).toBeLessThan(100);
  expect(layout.recommendationBorder).toBe('1px');
});
